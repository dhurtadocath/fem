"""
collect_gnn_newton_data.py — Data collection for GNN Newton step predictor
==========================================================================
Copy of ContactPotato_NGSolve.py instrumented to collect training data
for the GNN Newton Step Predictor (Phase 4).

At each load step's first Newton iteration (iteration 0), records:
  - u_current    (3*nv,)  — displacement at start of step
  - delta_u      (3*nv,)  — first Newton increment (Pardiso solution = GNN target)
  - residual     (3*nv,)  — residual vector at iteration 0
  - contact state: active mask, gaps, normals
  - load_fraction, deformed positions, Dirichlet info

Forces classical solver mode (no NN contact, no NN return mapping) to
generate clean reference data.

Usage:
  python collect_gnn_newton_data.py --n 5 --nsteps 100 --output-dir nn_contact/data/gnn_newton_raw
  python collect_gnn_newton_data.py --n 10 --nsteps 100 --plastic true
  python collect_gnn_newton_data.py --n 5 --nsteps 50 --E 0.03 --nu 0.35
"""
...
_parser.add_argument("--E", type=float, default=None, help="Young's modulus")
_parser.add_argument("--nu", type=float, default=None, help="Poisson ratio")
_parser.add_argument("--kn-factor", type=float, default=None, help="Penalty stiffness factor")
_parser.add_argument("--output-dir", type=str, default="nn_contact/data/gnn_newton_raw",
...
# Solver — ALWAYS Newton, classical mode for clean reference data
solver      = "newton"
nsteps      = 100           # number of load steps
max_iter    = 200            # max iterations per step
gtol        = 1e-12          # gradient/residual tolerance
max_cutback = 5             # max bisection levels on non-convergence

# Hessian
full_hessian = False         # include curvature term dn/dx_s in contact Hessian

# Output — No VTK, minimal overhead for data collection
compare     = False
plot        = 0             # VTK disabled for data collection
vtk_fields  = "minimal"
...
consistent_tangent = False  # off for data collection (faster)

# FORCED CLASSICAL: no NN contact, no NN return mapping
nn_contact          = False
nn_contact_mode     = "off"
nn_contact_device   = "cpu"
nn_contact_topk     = 3
nn_contact_checkpoint = None

nn_return_mapping      = False
nn_rm_checkpoint       = None
nn_rm_device           = "cpu"
nn_rm_autodiff_tangent = False

# ── GNN data collection state ─────────────────────────────────────────────────
_gnn_data_samples = []   # list of dicts, one per load step
_gnn_data_output_dir = os.path.abspath(_cli.output_dir)  # resolve BEFORE chdir
...
if _cli.E is not None:
    E_val = _cli.E
if _cli.nu is not None:
    nu_val = _cli.nu
if _cli.kn_factor is not None:
    kn_factor = _cli.kn_factor
# ──────────────────────────────────────────────────────────────────────────────
...
# chdir to the 1_Minimization_solvers/ directory where Dat/ and other
# relative paths are expected (this script is a copy from there)
_sim_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        '..', '..', '1_Minimization_solvers')
os.chdir(os.path.abspath(_sim_dir))
...
def newton_solve():
    ...
    for nit in range(max_iter):
        ...
        # ── GNN DATA COLLECTION (iteration 0 only) ────────────────────────
        if nit == 0 and solve_ok:
            _u_vec = gfu.vec.FV().NumPy()
            _w_np = _w_vec.FV().NumPy()
            # Deformed positions: X_ref + [u_x, u_y, u_z] per vertex
            _x_def = X_ref.copy()
            _x_def[:, 0] += _u_vec[:nv]
            _x_def[:, 1] += _u_vec[nv:2*nv]
            _x_def[:, 2] += _u_vec[2*nv:3*nv]

            # Dirichlet BC encoding: which nodes are constrained + prescribed value
            _is_dirichlet = np.zeros(nv, dtype=np.float32)
            _is_dirichlet[top_verts] = 1.0
            _bc_value = np.zeros(nv, dtype=np.float32)
            # BC value = magnitude of total prescribed displacement so far
            _bc_value[top_verts] = np.sqrt(
                _u_vec[top_verts]**2 + _u_vec[top_verts + nv]**2 +
                _u_vec[top_verts + 2*nv]**2)

            # Contact state for ALL slave nodes (not just active)
            _n_slave = len(slave_verts)
            _contact_flag = np.zeros(_n_slave, dtype=np.float32)
            _gap_slave = np.zeros(_n_slave, dtype=np.float32)
            _normal_slave = np.zeros((_n_slave, 3), dtype=np.float32)
            if gn_out is not None:
                _contact_flag[active_idx] = 1.0
                _gap_slave[:] = gn_out
                _normal_slave[:] = normals_out

            sample = {
                'u_current':     _u_vec.copy().astype(np.float32),
                'delta_u':       _w_np.copy().astype(np.float32),
                'residual':      r_np.copy().astype(np.float32),
                'x_deformed':    _x_def.astype(np.float32),
                'load_fraction': float(load + dt),  # load AFTER BC increment
                'is_dirichlet':  _is_dirichlet,
                'bc_value':      _bc_value,
                'contact_flag':  _contact_flag,
                'gap':           _gap_slave,
                'normal':        _normal_slave,
                'slave_verts':   slave_verts.copy(),
            ...
            if plastic:
                sample['epcum'] = _epcum_conv.copy().astype(np.float32)
            _gnn_data_samples.append(sample)
        # ───────────────────────────────────────────────────────────────────
...
# GNN DATA SAVE
# ══════════════════════════════════════════════════════════════════════════════
if _gnn_data_samples:
    os.makedirs(_gnn_data_output_dir, exist_ok=True)

    # Build filename with simulation parameters for easy identification
    _tag = f"_n{n}_ns{nsteps}_E{E_val}_nu{nu_val}_kn{kn_factor}"
    _tag += "_plastic" if plastic else "_elastic"
    if _cli.tag:
        _tag += f"_{_cli.tag}"
    _save_path = os.path.join(_gnn_data_output_dir,
                              f"gnn_newton_data{_tag}.npz")

    # Also store graph topology (edge_index) and static features
    # Build hex mesh edge connectivity (deduplicated, bidirectional)
    _edges = set()
    for el in mesh.Elements(VOL):
        ...
        for i in range(len(verts)):
            for j in range(i + 1, len(verts)):
                ...
                _edges.add((a, b))
    _edge_array = np.array(sorted(_edges), dtype=np.int32)  # (n_edges, 2)

    # Edge features (static): Δx_ref and ||Δx_ref||
    _dx_ref = X_ref[_edge_array[:, 1]] - X_ref[_edge_array[:, 0]]  # (n_edges, 3)
    _edge_len = np.linalg.norm(_dx_ref, axis=1, keepdims=True)      # (n_edges, 1)
    _edge_feat = np.hstack([_dx_ref, _edge_len]).astype(np.float32)  # (n_edges, 4)

    # Pack all samples
    _n_samples = len(_gnn_data_samples)
    _save_dict = {
        # Graph topology (static)
        'edge_index': _edge_array,      # (n_edges, 2) — undirected, need to add reverse
        'edge_features': _edge_feat,    # (n_edges, 4)
        'X_ref': X_ref.astype(np.float32),  # (nv, 3)
        'free_dofs': free_dofs,
        'slave_verts': slave_verts,
        'top_verts': top_verts,
        'nv': nv,
        'n': n,
    ...
    # Per-sample arrays — stack into (n_samples, ...) arrays
    for key in ['u_current', 'delta_u', 'residual', 'load_fraction',
                'is_dirichlet', 'bc_value', 'contact_flag', 'gap', 'normal',
                'x_deformed']:
        _save_dict[key] = np.array([s[key] for s in _gnn_data_samples])

    if plastic and 'epcum' in _gnn_data_samples[0]:
        _save_dict['epcum'] = np.array([s['epcum'] for s in _gnn_data_samples])

    np.savez_compressed(_save_path, **_save_dict)
    print(f"\nGNN data saved: {_save_path}")
    print(f"  {_n_samples} samples, {len(_edges)} undirected edges, {nv} vertices")
    print(f"  Per-sample: u({3*nv}), delta_u({3*nv}), residual({3*nv}), "
          f"contact({len(slave_verts)} slave nodes)")
else:
    print("\nWARNING: No GNN data samples collected!")
