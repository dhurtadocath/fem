...
def minimize_u_NR(u0,s,fr):
    pass


def minimize_u(u0,s,fr):
    alpha_init = 1
    c, r = 0.5, 0.5
    nreset = 10
...
while t+dt<=1.0+1e-4:
    ...
    # set_trace()
    # s0 = s                                                #4222 NM_oters fro dt=0.01
    s0 = s if xp is None else xp                          #4212 NM_iters for dt=0.01
    # s0 = s+ds if xp is None else xp                          #4212 NM_iters for dt=0.01
    # s0 = s + np.average(du.reshape(-1,2),axis=0)[0]       #4309 NM_iters for dt=0.01
    ds0=1e-8

    NM = minimize(mNM,s0,method='Nelder-Mead',args=(u,fr,xp,yc,Ft,Tau),options={'disp':False,'bounds':[(0,1)], 'initial-simplex':(s0-ds0,s0+ds0)},tol=1e-8)
    # NM = minimize(mNM,s0,method='Powell',args=(u,fr,xp,yc,Ft,Tau),options={'disp':False},tol=1e-12)
    set_trace()
    s = round(NM.x[0],10)
    print("DS  : " , ds)
    NI.append(NM.nit)
...
print("Total iterations for Nelder-Mead:", sum(NI))
