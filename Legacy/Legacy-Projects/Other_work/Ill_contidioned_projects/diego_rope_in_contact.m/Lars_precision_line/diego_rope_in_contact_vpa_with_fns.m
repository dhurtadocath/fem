...
% FE_node_coord=vpa((0:1:nrx-1)');
FE_node_coord=vpa([(0:1:nrx-1)' zeros(nrx,2)]);

FE_node_nrs=vpa([(1:nrx-1)' (2:nrx)']);
len_el=nrx-1;
len_u=nrx*3;
...
for incr=1:tot_incr
    ...
    while YN==1
        ...
        u(2)=vpa(incr/tot_incr*Utot);
        u(nrx*3-1)=vpa(incr/tot_incr*Utot);
        ...
        m_old=1e100;
        iter=0;
                while norm(f_2-f_new)>0 && norm(f_2)>0 && iter<100
