...
load('1/saved_vars');         % change so that it opens only the necessary vars
sph{1}{3} = sph{1}{3}+0.2;

ACTIVES1 = readmatrix('1/actives.csv');
DISPLACEMENTS1 = readmatrix('1/displacements.csv');
FORCES1 = readmatrix('1/forces.csv');

STRAINS1 = get_deformations(DISPLACEMENTS1,X,conn);


load('2/saved_vars');         % change so that it opens only the necessary vars

ACTIVES2 = readmatrix('2/actives.csv');
DISPLACEMENTS2 = readmatrix('2/displacements.csv');
FORCES2 = readmatrix('2/forces.csv');


STRAINS2 = get_deformations(DISPLACEMENTS2,X,conn);



n_times = size(ACTIVES1,1);
DOFS_BASE = {dofs_base_x,dofs_base_y,dofs_base_z};
DOF_SLAVES = dofs(slaves,:);

FN1 = zeros(n_times,ns);
FT1 = zeros(n_times,ns);
for time=1:n_times
    for idx_slave=1:ns
        if ACTIVES1(time,idx_slave)
            fnode= FORCES1(time,DOF_SLAVES(idx_slave,:));
            xs = X(slaves(idx_slave),:)+DISPLACEMENTS1(time,DOF_SLAVES(idx_slave,:));
            nor = (xs-cell2mat(sph{1}))/norm(xs-cell2mat(sph{1}));
            fn = fnode*nor';
            FN1(time,idx_slave) = fn;
            FT1(time,idx_slave) = norm(fnode-fn*nor);
        end
    end
end
FN2 = zeros(n_times,ns);
FT2 = zeros(n_times,ns);
for time=1:n_times
    for idx_slave=1:ns
        if ACTIVES1(time,idx_slave)
            fnode= FORCES2(time,DOF_SLAVES(idx_slave,:));
            xs = X(slaves(idx_slave),:)+DISPLACEMENTS2(time,DOF_SLAVES(idx_slave,:));
            nor = (xs-cell2mat(sph{1}))/norm(xs-cell2mat(sph{1}));
            fn = fnode*nor';
            FN2(time,idx_slave) = fn;
            FT2(time,idx_slave) = norm(fnode-fn*nor);
        end
    end
end
...
hold on;
plot(1:n_times1, sum(FORCES1(:,dofs_base_x), 2), 'b');
plot(1:n_times1, sum(FORCES1(:,dofs_base_y), 2), 'g');
plot(1:n_times1, sum(FORCES1(:,dofs_base_z), 2), 'r');
plot(1:n_times2, sum(FORCES2(:,dofs_base_x), 2), 'b*');
plot(1:n_times2, sum(FORCES2(:,dofs_base_y), 2), 'g*');
plot(1:n_times2, sum(FORCES2(:,dofs_base_z), 2), 'r*');
...
%% Functions
function plot4d(ACTIVES,DISPLACEMENTS,FORCES,DOFS,STRAINS,slaves,sph,X,conn,n_times)
    % Create a figure
    ...
    plotHandles(1) = plot(1:n_times, sum(FORCES(:,dofs_base_x), 2), 'b');
    plotHandles(2) = plot(1:n_times, sum(FORCES(:,dofs_base_y), 2), 'g');
    plotHandles(3) = plot(1:n_times, sum(FORCES(:,dofs_base_z), 2), 'r');
    legend('\SigmaFx','\SigmaFy','\SigmaFz','Location','northwest');
