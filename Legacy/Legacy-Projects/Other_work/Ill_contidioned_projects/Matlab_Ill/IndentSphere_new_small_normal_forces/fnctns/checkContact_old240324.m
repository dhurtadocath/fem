function [act, shnew, Redo] = checkContact(u, sph, idx_act, sh, s, X, dofs, slaves, conn, ns, mu,ratio)
    ...
    for idx = 1:length(xs)
        ...
        % if gn > 0  || fnode'*nor>1e-10 % slave is out
        % if gn > 1e-18  || fnode'*nor>1e-10 % slave is out
        % if gn > 1e-10  || fnode'*nor<-1e-10 % slave is out
        if gn > 1e-20
        % if fnode'*nor<-1e-10 % slave is out
            ...
            if ismember(idx, idx_act)   % was active
                ...
                Redo = true;
                fprintf('node %d immediate first slide\n', slaves(idx));
                % sh(idx,:) = si;
