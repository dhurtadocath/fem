function [cc,asd,u,dccds,sdf]=nonlcon(s,u0,X,conn,fr,idx_act,mu,ratio,k_pen,sph,slaves,dofs,sh,print_stuff)
...
% function [cc,cl,dccds,dclds]=nonlcon(s,u0,X,conn,fr,idx_act,mu,ratio,k_pen,sph,slaves,dofs,sh)
    ...
    if isnan(uN)
        ...
        % toc
        % tic
        % [u,~,~,normF] = fullBFGStoCompileToMex(u0,X,conn,is_free,is_active,mu,ratio,k_pen,Cx,Cy,Cz,R,slaves,dofs,s_full,sh);
        % toc
        [u,~,normF] = fullBFGStoCompileToMex_mex(u0,X,conn,is_free,is_active,mu,ratio,k_pen,Cx,Cy,Cz,R,slaves,dofs,s_full,sh);
        % update_plot(normF)
    ...
    for ids=idxs_hooked
        ...
        if (xch-xc)==0.0
            ...
            % cl(cnt_act) =  norm(ft/Ft-Tau);
            % cl(cnt_act) =  acos((ft'*Tau)/Ft);
            cl(cnt_act) =  -ft'*Taup/Ft;
            if print_stuff
                fprintf("[d\x03B8=%0.4f°, ",acos((ft'*Tau)/Ft)*180/pi)
            end
...
function update_plot(normF)
    ...
    % Append new values
    BFGS_res = [BFGS_res, normF];
    iteration = length(BFGS_res);

    % Determine color based on eval_situation
    if eval_situation == 0
        color = [0.5 0.5 0.5]; % Gray
    elseif eval_situation == 1
        color = [1 0.5 0]; % Orange
    elseif eval_situation == 2
        color = [0 0 1]; % Blue
    elseif eval_situation == 3
        color = [0 1 0]; % Green
    end

    % Determine size of the dot based on eval_situation
    if eval_situation == 3
        size_dot = 50; % Larger size
    else
        size_dot = 20; % Smaller size
    end

    % Ensure the scatter plot updates in the dedicated figure
    figure(scatter_figure);
    hold on;
    scatter(iteration, normF, size_dot, 'MarkerFaceColor', color, 'MarkerEdgeColor', color);
    hold off;
    drawnow;
end
