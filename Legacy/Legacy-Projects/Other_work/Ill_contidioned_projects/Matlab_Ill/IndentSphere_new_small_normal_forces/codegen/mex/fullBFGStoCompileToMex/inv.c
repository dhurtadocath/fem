                   ...
                   emxArray_real_T *y)
{
  ...
  for (k = 0; k < n_tmp; k++) {
    ...
    for (yk = k + 1; yk <= n_tmp; yk++) {
      if (y_data[(yk + y->size[0] * (i1 - 1)) - 1] != 0.0) {
        i = yk + 1;
        for (pipk = i; pipk <= n_tmp; pipk++) {
          y_data[(pipk + y->size[0] * (i1 - 1)) - 1] -=
              y_data[(yk + y->size[0] * (i1 - 1)) - 1] *
              b_x_data[(pipk + b_x->size[0] * (yk - 1)) - 1];
        }
      }
    }
  }
