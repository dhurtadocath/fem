...
ACTIVES=ACTIVES(1:94,:);
DISPLACEMENTS=DISPLACEMENTS(1:94,:);
FORCES=FORCES(1:94,:);
...
function plot4d(ACTIVES,DISPLACEMENTS,FORCES,DOFS,STRAINS,slaves,sph,X,conn,n_times)
    ...
    plotHandles(1) = plot(1:n_times, 4*sum(FORCES(:,dofs_base_x), 2), 'b');
    plotHandles(2) = plot(1:n_times, 4*sum(FORCES(:,dofs_base_y), 2), 'g');
    plotHandles(3) = plot(1:n_times, 4*sum(FORCES(:,dofs_base_z), 2), 'r');
