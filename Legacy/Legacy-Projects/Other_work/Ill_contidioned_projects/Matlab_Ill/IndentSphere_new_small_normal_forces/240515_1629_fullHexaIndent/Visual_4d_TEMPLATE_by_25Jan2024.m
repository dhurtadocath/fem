...
plot4d(ACTIVES,DISPLACEMENTS,FORCES,DOFS,slaves,sph,X,conn,n_times);



%% Functions
function plot4d(ACTIVES,DISPLACEMENTS,FORCES,DOFS,slaves,sph,X,conn,n_times)
    % Create a figure
    ...
    % Initial drawing
    currentTimePointHandles = redraw_figure(scrollbar, ax, ax2, plotHandles, currentTimePointHandles, DISPLACEMENTS, ACTIVES, slaves, sph, X, conn, n_times, currentValueLabel);


    function callbackFunction(src)
        % Update currentTimePointHandles with each call
        currentTimePointHandles = redraw_figure(src, ax, ax2, plotHandles, currentTimePointHandles, DISPLACEMENTS, ACTIVES, slaves, sph, X, conn, n_times, currentValueLabel);
    end


end

function currentTimePointHandles = redraw_figure(scrollbar, ax, ax2, plotHandles,currentTimePointHandles,DISPLACEMENTS,ACTIVES,slaves,sph,X,conn,n_times,currentValueLabel)
    ...
    % Sphere's position
    sph{1}{3} = sph{1}{3}+(-0.9)*(time)/n_times;
    ...
    ax = plotTruss_XY_symm(DISPLACEMENTS(time,:)',X,conn,actives,ax);
