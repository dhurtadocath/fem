...
FNs = zeros(ns,n_times);
FTs = zeros(ns,n_times);
ANGs= zeros(ns,n_times);

for tt=1:n_times
    sl_idx = 1;
    sph{1}{3} = Cz+(-0.9)*(tt)/n_times
    for ids=slaves
        actives = slaves(ACTIVES(tt,:)==1);
        if ismember(ids,actives)
            xsi = X(ids,:)+DISPLACEMENTS(tt,dofs(ids,:));
            si = getProjs(xsi,sph);
            xyz_sph = cell2mat(sph{1});
            dcs = xsi-xyz_sph;
            nor = dcs/norm(dcs);
            f_node = FORCES(tt,dofs(ids,:));
            Fn = f_node*nor';
            ft = f_node-Fn*nor;
            Ft = norm(ft);
            ...
            FNs(sl_idx,tt)=Fn;
            FTs(sl_idx,tt)=Ft;
            xsi_prev = X(ids,:)+DISPLACEMENTS(tt-1,dofs(ids,:));
            dxs = xsi_prev-xsi;
            dxst = dxs-(dxs*nor')*nor;
            Tau = dxst/norm(dxst);
            ANGs(sl_idx,tt)=acos((-ft*Tau')/Ft)*180/pi;
...
plot4d(ACTIVES,DISPLACEMENTS,FORCES,DOFS,STRAINS,slaves,sph,X,conn,n_times,nodes_base);



%% Functions
function plot4d(ACTIVES,DISPLACEMENTS,FORCES,DOFS,STRAINS,slaves,sph,X,conn,n_times,nodes_base)
    % Create a figure
    ...
    plotHandles(1) = plot(1:n_times, sum(abs(FORCES(:,dofs_base_x)), 2), 'b');
    plotHandles(2) = plot(1:n_times, sum(abs(FORCES(:,dofs_base_y)), 2), 'g');
    ...
    % Initial drawing
    currentTimePointHandles = redraw_figure(scrollbar, ax, ax2, plotHandles, currentTimePointHandles, DISPLACEMENTS, ACTIVES, slaves, sph, X, conn, n_times, currentValueLabel,STRAINS,nodes_base);


    function callbackFunction(src)
        % Update currentTimePointHandles with each call
        currentTimePointHandles = redraw_figure(src, ax, ax2, plotHandles, currentTimePointHandles, DISPLACEMENTS, ACTIVES, slaves, sph, X, conn, n_times, currentValueLabel,STRAINS,nodes_base);
    end


end

function currentTimePointHandles = redraw_figure(scrollbar, ax, ax2, plotHandles,currentTimePointHandles,DISPLACEMENTS,ACTIVES,slaves,sph,X,conn,n_times,currentValueLabel,STRAINS,nodes_base)
    ...
    ax = hggroup();
    ax = plotTruss(DISPLACEMENTS(time,:)',X,conn,actives,ax,STRAINS,time);
    ax = plotsphere(sph,ax);
    scatter3(X(nodes_base,1),X(nodes_base,2),X(nodes_base,3),"black","filled");
