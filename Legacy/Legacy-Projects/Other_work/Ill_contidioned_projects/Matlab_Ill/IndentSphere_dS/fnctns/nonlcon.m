...
% function [cc,cl,dccds,dclds]=nonlcon(s,u0,X,conn,fr,idx_act,mu,ratio,k_pen,sph,slaves,dofs,sh)
    ...
    for ids=idxs_hooked
        ...
        if nargout>2 && norm(f)>0
            ...
            % self relation force_i-projection_i
            ...
            dccdf=dccdFt*dFtdft*(dftdf+dftdfn*dfndFn*dFndf)+dccdabsFn*dabsFndFn*dFndf;
            dccds(cnt_act,[cnt_act,nh+cnt_act]) = dccds(cnt_act,[cnt_act,nh+cnt_act]) ...
                + dccdf*dfdu*duds;


            % neighbours relation force_j-projection_i
            neighbor_nodes = cell2mat(conn_slaves(ids));
            for neigh = neighbor_nodes
                jdx = find(idxs_hooked==neigh);  % relative position of neighbour
                if ~isempty(jdx)
                    dofj = dofs(neigh,:);
                    dfiduj = kint(dofi,dofj);
                    [~,dndsj] = get_normals(s(jdx,:));
                    dudsj = R*dndsj;
    
                    dccds(cnt_act,[jdx,nh+jdx]) = dccds(cnt_act,[jdx,nh+jdx]) ...
                        + dccdf*dfiduj*dudsj;
                end
