function [s_opt,cnt] = CG_mf_constr_ParLnSrch(u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,s_opt,sh,ndpls)
    s_size = size(s_opt);
    ...
    while (mold-mnew)>tol*abs(mold)
        ...
        % fnew = dmm( s ,u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,sh,k_op)';
       ...
       if mod(cnt,nreset)==0
        ...
        while norm((alpha_b-alpha_a)*hnew)>1e-8
            ...
            parfor i = 1:ndpls
                ...
                alpha = alphas(i)
                ms(i) = mm( s+alpha*hnew ,u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,sh,k_op);
...
function m_constr = mm(s_opt,u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,sh,k_op)
    ...
    % m_constr = 1/2*k_op*(c_in'*c_in);
end

function dm_constr = dmm(s_opt,u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,sh,k_op)
    s_opt = reshape(s_opt,[],2);
    [c_ineq,c_eq,dc_in,dc_eq] = nonlcon(s_opt,u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,sh);
    c_in = max(c_ineq,0);
    [~,d_obj] = mgt2(s_opt,u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,sh);

    % dm_constr = reshape(d_obj,1,[]) + k_op*c_eq*dc_eq' + k_op*(c_in'*dc_in);
    dm_constr = reshape(d_obj,1,[]) + k_op*c_eq*dc_eq' + 2*k_op*(c_in'*dc_in);
    % dm_constr = k_op*(c_in'*dc_in);
