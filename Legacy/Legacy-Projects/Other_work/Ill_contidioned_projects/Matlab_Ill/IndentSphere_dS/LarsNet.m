...
tic
global conn_slaves;
conn_slaves = cell(ns,1);
for i = 1:length(slaves)
    slave = slaves(i);
    flat_pairs = reshape(conn(any(conn == slave, 2),:),1,[]);
    conn_slaves{i} = flat_pairs(flat_pairs~=slave);
end
toc 
...
for incr=(latest_incr+1):nincr
    ...
    while Redo==1
        ...
        if ~isempty(idxs_hooked)
            ...
            % [s_opt,iters] = CG_mf_constr(u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,s_opt,sh);
            [s_opt,iters] = CG_mf_constr_ParLnSrch(u,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,s_opt,sh);
        ...
        if isnan(uN)
            % ucax = BFGScache(@(uv) mf_with_constr(uv,X,conn,free_ind,actives,mu,ratio,k_pen,sph,slaves,dofs,s_opt,sh),u,free_ind);
            ...
            % u = fullBFGStoCompileToMex_mex(u,X,conn,is_free,is_active,mu,ratio,k_pen,Cx,Cy,cz,R,slaves,dofs,s_full,sh);
            u = new_fullBFGStoCompileToMex(u,X,conn,is_free,is_active,mu,ratio,k_pen,Cx,Cy,Cz,R,slaves,dofs,s_full,sh);
