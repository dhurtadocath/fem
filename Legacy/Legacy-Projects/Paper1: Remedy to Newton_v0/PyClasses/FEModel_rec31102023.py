...
class FEModel:
    ...
    def solve_NR(self,tol=1e-10,maxiter=10,plotIters=False):
        ...
        TimeDisp = True
        RES, niter, res_pre = 1+tol, 0, 0.0

        # if any(self.contacts[0].actives): set_trace()
    ...
    def Solve(self, t0 = 0, tf = 1, TimeSteps = 1, max_iter=10 , recover = False, ForcedShift = False):
        ...
        while t+dt < tf+1e-4:
            ...
            ### Solver Algorithm ###
            ########################
            converged = self.solve_NR(tol=tolerance)
            # converged = self.solve_BFGS(tol=tolerance)

            # if abs(t-0.465)<0.0050001:
            #     set_trace()
