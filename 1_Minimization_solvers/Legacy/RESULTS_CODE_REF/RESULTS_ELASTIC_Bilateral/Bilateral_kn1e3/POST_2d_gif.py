...
def generate_gif(model, TIMES, UU, SED, plastic,gif_filename='animation.gif'):
    ...
    # Read the data from 'data_Redos_due_to_kn_adjustment.csv'
    data = pd.read_csv(os.path.join(cwd,'data_Redos_due_to_kn_adjustment.csv'), header=None)
    times_redos = data[0]
    redos = data[1]
    ...
    for idx, time in enumerate(TIMES):
        ...
        if (idx%1==0) or (abs(100 * time - round(100 * time)) < 1e-10) or (idx==len(TIMES)-1):  # Only consider integer time steps
            ...
            try:
                ax_fx.scatter([time], [sumFx[np.argmin(abs(time-times))]], color='blue', s=50) # was 1e-12
            except:
                import pdb; pdb.set_trace()
            ax_fx.set_xlim(0, 1)
            ...
            ax_fz.scatter([time], [sumFz[np.argmin(abs(time-times))]], color='blue', s=50)
            ax_fz.set_xlim(0, 1)
            ax_fz.set_xlabel(r'$\mathrm{Time}$')
            ax_fz.set_ylabel(r'$\mathrm{Fz}$')
            ax_fz.set_title(r'$\mathrm{Vertical\ force}$')

            reps = redos[idx]+1 if exact_time else 1
            for rep in range(reps):
                # Save the frame
                if reps>1:
                    ax.set_facecolor((1.0, 1.0, 0.8))  # Pale yellow
    ...
    imageio.mimsave(os.path.join(cwd, gif_filename), images, fps=14, loop=0)  # Adjust fps as needed
