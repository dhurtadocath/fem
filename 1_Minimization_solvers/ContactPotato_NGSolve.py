...
# Energy CF defined on gfu (for Integrate)
F_gfu  = Id(3) + Grad(gfu)
J_gfu  = Det(F_gfu)
I1_gfu = Trace(F_gfu.trans * F_gfu)
psi_cf = c10 * (I1_gfu - 3 - 2*log(J_gfu)) + d1 * log(J_gfu)**2
...
def compute_contact(slave_pos):
    ...
    for i in range(n_slave):
        ...
        # KD-tree candidates
        k_query    = min(K_SURF, len(surf_pts))
        _, idx_kd  = surf_kdtree.query(xsi, k=k_query)
        ...
        for attempt in range(2):
            ...
            best_patch, t1, t2, _ = gb.find_projection_tr_multi(
                ctrlpts_all, xsi.astype(np.float64),
                merged.astype(np.int32), radii, eps,
...
def objective(x_free):
    ...
    E_mat = Integrate(psi_cf * dx, mesh)
    ...
    # --- Contact penalty ---------------------------------------------------
    gn, normals, active = compute_contact(slave_pos)
